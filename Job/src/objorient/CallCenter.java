...
public final class CallCenter {
	...
	public static final class CallCenterSystem {
		...
		private final ArrayBlockingQueue<Call> callQueue = new ArrayBlockingQueue<Call>(25);
		
		// This isn't used for anything at the moment. Could be used for tracking, such as
		// average call time to estimate wait time.
//		private final ConcurrentHashMap<String, CallHandler> allAgents
//				= new ConcurrentHashMap<String, CallHandler>();
		
		private final ConcurrentLinkedQueue<CallHandler> openAgents
				= new ConcurrentLinkedQueue<CallHandler>();
				...
				public void run() {
					while (!Thread.currentThread().isInterrupted()) {
						...
						if (agent == null)
							...
							// This would have to be handled in a non-blocking way
		...
		public void login(CallHandler agent, EmployeeType role) {
			...
			case PM:
				if (productManager == null)
					...
					throw new RuntimeException("Only one Product Manager can be logged in at one time.");
				break;
			...
			case TL:
				if (technicalLead == null)
					...
					throw new RuntimeException("Only one Technical Lead can be logged in at one time.");
				break;

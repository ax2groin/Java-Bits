...
public final class ParallelInitializer {
  ...
  private void initializeObjects() {
    ...
    int threadPoolSize = Math.min(objUIDs.length, Runtime.getRuntime().availableProcessors());
      ...
      completionService.submit(new Callable<TimeConsumingObject>() {
        public TimeConsumingObject call() {
          try {
            ...
            // TODO: Never catch "Exception, but catch any relevant exceptions here
            // Log error: "Failed to initialize TimeConsumingObject for " + objName, e
            return null;
          }
        }
      });
    ...
    for (int i = 0; i < objUIDs.length; i++)
      try {
        ...
        timeConsumingObjects.put(timeConsumingObject.getUID(), timeConsumingObject);
  ...
  public static final class TimeConsumingObject {
    ...
    public TimeConsumingObject(String uid) {
      this.uid = uid;
      // And then some calculations, state initialization, etc. that takes a long time.
    }
